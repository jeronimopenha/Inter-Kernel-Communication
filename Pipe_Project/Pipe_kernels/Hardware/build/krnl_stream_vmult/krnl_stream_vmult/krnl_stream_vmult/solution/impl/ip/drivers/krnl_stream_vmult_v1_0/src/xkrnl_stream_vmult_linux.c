...
typedef struct {
    ...
    XKrnl_stream_vmult_uio_map maps[ MAX_UIO_MAPS ];
} XKrnl_stream_vmult_uio_info;

/***************** Variable Definitions **************************************/
static XKrnl_stream_vmult_uio_info uio_info;
...
static int uio_info_read_name(XKrnl_stream_vmult_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKrnl_stream_vmult_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKrnl_stream_vmult_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKrnl_stream_vmult_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKrnl_stream_vmult_Initialize(XKrnl_stream_vmult *InstancePtr, const char* InstanceName) {
	XKrnl_stream_vmult_uio_info *InfoPtr = &uio_info;
